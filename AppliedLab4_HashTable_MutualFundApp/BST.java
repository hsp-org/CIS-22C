...
public class BST<T> {
    ...
    public T sharedPrecursor(T data1, T data2, Comparator<T> cmp) throws IllegalArgumentException {
        ...
        return sharedPrecursor(data1, data2, root, cmp).data;
    }
     ...
     */
    private Node sharedPrecursor(T data1, T data2, Node node, Comparator<T> cmp) {
        if (node == null) {
            return null;
        }
        int cmpData1 = cmp.compare(data1, node.data);
        int cmpData2 = cmp.compare(data2, node.data);

        if (cmpData1 < 0 && cmpData2 < 0) {
